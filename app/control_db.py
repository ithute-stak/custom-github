...
class CompatCursor:
    def __init__(self, connection: "PostgresCompatConnection", cursor: psycopg.Cursor | None = None):
        ...
        self._description_names: list[str] = []
        self.rowcount = -1
    ...
    def description(self):
        ...
        if self._synthetic is not None and self._synthetic:
            # SQLite callers generally use description only for column names.
...
class PostgresCompatConnection:
    ...
    def _statement(self, query: str) -> str:
        ...
        if re.match(r"^\s*INSERT\s+INTO\s+", value, re.I) and " OR IGNORE " not in query.upper() and "ON CONFLICT" in value.upper():
            pass
        elif re.match(r"^\s*INSERT\s+INTO\s+", value, re.I) and re.match(r"^\s*INSERT\s+OR\s+IGNORE\s+", query, re.I):
