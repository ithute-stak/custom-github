...
from app.github_admin_center import GitHubAdminAPI, install_github_admin_routes
...
class FakeAdminAPI:
    ...
    def get(self, path: str):
        ...
        if path.endswith("/actions/variables?per_page=100"):
            return {"variables": [{"name": "DEPLOY_REGION", "value": "ls-maseru", "created_at": "2026-09-01T00:00:00Z", "updated_at": "2026-09-20T00:00:00Z"}]}
        if path.endswith("/actions/secrets?per_page=100"):
            return {"secrets": [{"name": "DEPLOY_TOKEN", "created_at": "2026-09-01T00:00:00Z", "updated_at": "2026-09-20T00:00:00Z", "value": "MUST_NOT_LEAK"}]}
        if path.endswith("/environments?per_page=100"):
            return {"environments": [{"id": 8, "name": "production", "url": "api", "html_url": "html", "created_at": "2026-09-01T00:00:00Z", "updated_at": "2026-09-20T00:00:00Z", "protection_rules": [{"type": "wait_timer"}], "deployment_branch_policy": {"protected_branches": True}}]}
        if path.endswith("/actions/caches?per_page=100"):
            return {"actions_caches": [{"id": 41, "ref": "refs/heads/main", "key": "pip-linux", "version": "v1", "size_in_bytes": 1048576, "created_at": "2026-09-20T00:00:00Z", "last_accessed_at": "2026-09-21T00:00:00Z"}]}
        if path.endswith("/actions/runners?per_page=100"):
            return {"runners": [{"id": 9, "name": "build-01", "os": "linux", "status": "online", "busy": False, "labels": [{"name": "self-hosted"}, {"name": "linux"}]}]}
        if path.endswith("/deployments?per_page=50"):
            return [{"id": 11, "environment": "production", "ref": "main", "sha": "a" * 40, "task": "deploy", "created_at": "2026-09-21T00:00:00Z", "updated_at": "2026-09-21T00:01:00Z", "creator": {"login": "tester"}}]
        if "/dependabot/alerts?" in path:
            return [{"number": 1, "state": "open", "dependency": {"package": {"name": "demo-lib"}, "manifest_path": "requirements.txt", "scope": "runtime"}, "security_advisory": {"severity": "high", "summary": "Upgrade dependency"}, "html_url": "dep"}]
        if "/code-scanning/alerts?" in path:
            return [{"number": 2, "state": "open", "rule": {"id": "py/demo", "description": "Unsafe call", "security_severity_level": "high"}, "tool": {"name": "CodeQL"}, "created_at": "2026-09-20T00:00:00Z", "html_url": "code"}]
        if "/secret-scanning/alerts?" in path:
            return [{"number": 3, "state": "open", "secret_type": "github_pat", "secret_type_display_name": "GitHub token", "secret": "ghp_THIS_MUST_NEVER_LEAK", "resolution": None, "created_at": "2026-09-20T00:00:00Z", "html_url": "secret"}]
        if "/environments/production/variables?" in path:
            return {"variables": [{"name": "API_URL", "value": "https://api.example.test", "created_at": "2026-09-01T00:00:00Z", "updated_at": "2026-09-20T00:00:00Z"}]}
        if "/environments/production/secrets?" in path:
            return {"secrets": [{"name": "PROD_PASSWORD", "created_at": "2026-09-01T00:00:00Z", "updated_at": "2026-09-20T00:00:00Z", "secret": "MUST_NOT_LEAK_EITHER"}]}
...
def test_variable_delete_requires_exact_confirmation(monkeypatch) -> None:
    ...
    bad = client.request(
        "DELETE",
        "/api/projects/1/github/admin/variables/OLD_VALUE",
    ...
    good = client.request(
        "DELETE",
        "/api/projects/1/github/admin/variables/OLD_VALUE",
        json={"confirmation": "DELETE VARIABLE OLD_VALUE"},
    )
...
def test_platform_admin_routes_are_unique() -> None:
    paths = [getattr(route, "path", "") for route in platform_app.router.routes]
    expected = {
        "/github/admin",
        "/projects/{project_id}/github/admin",
        "/api/projects/{project_id}/github/admin/capabilities",
        "/api/projects/{project_id}/github/admin/overview",
        "/api/projects/{project_id}/github/admin/environments/{environment_name}",
        "/api/projects/{project_id}/github/admin/variables/{name}",
        "/api/projects/{project_id}/github/admin/caches/{cache_id}",
    }
